...
def load_gpt_model_and_tokenizer(model_name:str, device='cuda'):
    ...
    if model_name == 'gpt2-xl':
        ...
        MODEL_CONFIG = {
            ...
            "resid_dim": model.config.hidden_size,
            "name_or_path": model.config.name_or_path,
            # GPT-Neo uses a similar architecture to GPT-2
            "attn_hook_names": [f'transformer.h.{layer}.attn.attention.out_proj' for layer in range(model.config.num_hidden_layers)],
            "layer_hook_names": [f'transformer.h.{layer}' for layer in range(model.config.num_hidden_layers)],
            "prepend_bos": False
        }
